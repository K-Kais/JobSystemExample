...
namespace NativeQuadTree
{
	public unsafe partial struct NativeQuadTree<T> where T : unmanaged
	{
		struct QuadTreeRangeQuery
		{
			...
			public void Query(NativeQuadTree<T> tree, QuadBounds bounds, NativeList<QuadElement<T>> results)
			{
				this.tree = tree;
				this.bounds = bounds;
				count = 0;
			...
			public void RecursiveRangeQuery(QuadBounds parentBounds, bool parentContained, int prevOffset, int depth)
			{
			...
			static QuadBounds GetChildBounds(QuadBounds parentBounds, int childZIndex)
			{
				var half = parentBounds.Extents.x * .5f;

				switch (childZIndex)
				{
					case 0: return new QuadBounds(new float2(parentBounds.Center.x - half, parentBounds.Center.y + half), half);
					case 1: return new QuadBounds(new float2(parentBounds.Center.x + half, parentBounds.Center.y + half), half);
					case 2: return new QuadBounds(new float2(parentBounds.Center.x - half, parentBounds.Center.y - half), half);
					case 3: return new QuadBounds(new float2(parentBounds.Center.x + half, parentBounds.Center.y - half), half);
